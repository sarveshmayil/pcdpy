...
impl PyMetadata {
    ...
    fn set_fields(&mut self, new_fields: Vec<String>) -> PyResult<()> {
        let mut md = self.inner.lock().unwrap();
    ...
    fn get_npoints(&self) -> usize {
        self.inner.lock().unwrap().npoints
    }
    ...
    #[setter]
    fn set_shape(&mut self, value: (usize, usize)) {
        assert_eq!(value.0 * value.1, self.inner.lock().unwrap().npoints, "Shape must match number of points");
        self.inner.lock().unwrap().width = value.0;
        self.inner.lock().unwrap().height = value.1;
    }
    ...
    fn get_encoding(&self) -> String {
        self.inner.lock().unwrap().encoding.as_str().to_string()
    }
